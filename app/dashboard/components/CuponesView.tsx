  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-foreground">{couponsText.title}</h3>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {cupones.map((cupon) => {
            ...
            const statusColor = !cupon.activo ? 'bg-muted text-muted-foreground' : isExpired ? 'bg-destructive/15 text-destructive' : isExhausted ? 'bg-warning/15 text-warning' : 'bg-success/15 text-success';

            return (
              <div key={cupon.id} className="flex items-center gap-3 p-4 rounded-2xl border bg-card shadow-sm">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="font-semibold text-foreground">{cupon.codigo}</p>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${statusColor}`}>{statusBadge}</span>
                  </div>
                  <p className="text-sm text-muted-foreground mb-1">{cupon.descripcion || '-'}</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="btn btn-ghost p-2 text-muted-foreground hover:text-destructive"
                  >
                    <TrashIcon size={16} />
                  </button>
                </div>
