...
></iframe>`;
  ...
  return (
    <div className="mt-6 border rounded-2xl overflow-hidden bg-card">
      {/* Header */}
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="flex items-center gap-3 p-3 bg-primary/10 rounded-xl border border-primary/20">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-primary">{d.embedWidget.widgetUrl}</p>
            <p className="text-xs text-primary/80 truncate mt-0.5">{widgetUrl}</p>
          </div>
          <button
            onClick={() => copyText(widgetUrl, 'url')}
            className={`btn btn-sm shrink-0 transition-all ${copied === 'url' ? 'bg-success/15 text-success border-success/20' : 'btn-secondary'}`}
          >
        ...
        <div className="flex items-center gap-3">
          <label className="text-xs font-semibold text-muted-foreground whitespace-nowrap">{d.embedWidget.iframeHeight}</label>
          <input
          ...
          />
          <span className="text-xs font-mono text-muted-foreground w-12 text-right">{iframeHeight}px</span>
        </div>
            ...
            {(Object.keys(PLATFORM_SNIPPETS) as EmbedPlatform[]).map(p => (
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all ${
                  platform === p ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
                }`}
              >
...
{current.code}
          ...
          <div className="mt-3 bg-warning/10 border border-warning/20 rounded-xl px-4 py-3">
            ...
            <ol className="text-xs text-warning/90 space-y-1 list-decimal list-inside">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs text-center text-muted-foreground">
          {[
            { icon: <RefreshIcon size={18} className="text-primary" />, text: d.embedWidget.syncsWithAgenda },
            { icon: <PhoneIcon size={18} className="text-primary" />, text: d.embedWidget.responsive },
            { icon: <ShieldIcon size={18} className="text-primary" />, text: d.embedWidget.noPatientReg },
          ].map(({ icon, text }) => (
            <div key={text} className="bg-muted/30 rounded-xl p-2.5 border">
        ...
        {preview && (
          <div className="space-y-2">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{d.embedWidget.livePreview}</p>
